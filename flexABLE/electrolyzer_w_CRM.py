...
class Electrolyzer():
    ...
    def __init__(self,
        ...
        self.dictCapacity_CRM = {n:0 for n in self.world.snapshots}
        self.CRM_EOM_difference = {n:0 for n in self.world.snapshots}
        self.sentBids = []
...
# manages the available capacity, energy storage (SOC), energy cost, and tracks the success of the market. 
    def step(self): 
        ...
        # self.dictCapacity_CRM[self.world.currstep] = 0 #It initializes the available capacity at the current time step to zero.
        for bid in self.sentBids: #why only EOM calculated here??
            if 'demandEOM' in bid.ID: #If the bid's ID contains the substring 'demandEOM', it decreases the available capacity at the current time step
                ...
                self.dictCapacity_CRM[self.world.currstep] = bid.confirmedAmount            
        print(self.dictCapacity_CRM[self.world.currstep], self.name, 'self.dictCapacity_CRM[self.world.currstep]')
        print(self.dictCapacity[self.world.currstep], self.name, 'self.dictCapacity[self.world.currstep]')
        
        # #keep track of produced H2 from CRM participation
        if self.world.currstep >= 1 and self.world.currstep < len(self.world.snapshots) - 1: #all simulation except the last
            optimization_results = pd.read_csv('output/{}/Elec_capacities/{}_optimizedBidAmount.csv'.format(self.world.scenario, self.name))
            opt_bidEOM =  optimization_results["bidQuantity"][self.world.currstep]
            opt_demand =  optimization_results["h2demand"][self.world.currstep]
            # opt_h2prod =   optimization_results["produced_h2"][self.world.currstep]          
            self.CRM_EOM_difference[self.world.currstep + 1] = max((self.CRM_EOM_difference[self.world.currstep] + self.dictCapacity_CRM[self.world.currstep] - opt_bidEOM), 0)
            # print(self.CRM_EOM_difference[self.world.currstep], self.name, 'self.CRM_EOM_difference[self.world.currstep]')
            compressor_Cons = self.dictCapacity_CRM[self.world.currstep] * self.effElec / self.energyContentH2_LHV*self.world.dt * self.comprCons
            self.dictSOC[self.world.currstep + 1] = self.dictSOC[self.world.currstep] + ((self.dictCapacity_CRM[self.world.currstep] - compressor_Cons) * self.effElec / self.energyContentH2_LHV * self.world.dt) - opt_demand
            print(self.dictSOC[self.world.currstep] , self.name, 'self.dictSOC[self.world.currstep]')
...
# generate and return a list of bids based on the specified market type ("EOM," or "negCRMDemand") and the current time step t
    ...
    def calculateBidEOM(self, t):
        ...
        if os.path.exists('output/{}/Elec_capacities/{}_optimizedBidAmount.csv'.format(self.world.scenario, self.name)):
            optimization_results = pd.read_csv('output/{}/Elec_capacities/{}_optimizedBidAmount.csv'.format(self.world.scenario, self.name))            
            opt_bidQuantity = optimization_results["bidQuantity"][t]             
            if self.CRM_EOM_difference[t] >= opt_bidQuantity and self.confQtyCRM_neg[t] != 0: #there is enough reserve and if bidding in CRM
                bidQuantity_demand = 0
            elif self.CRM_EOM_difference[t] >= opt_bidQuantity and self.confQtyCRM_neg[t] == 0: #there is enough reserve and if no bidding in CRM
                bidQuantity_demand = self.standbyCons
            elif self.CRM_EOM_difference[t] < opt_bidQuantity:
                bidQuantity_demand = opt_bidQuantity - self.CRM_EOM_difference[t] 
    ...
    #Industrial plants participare in CRM using its H2 system capacity
    #CRM participation is done starting from secon timestep
    def calculatingBids_CRM_neg(self, t):
        bidsCRM = []
        if t >= 1: #TODO
            availablePower_H2_storage = min(max(((self.maxSOC - self.dictSOC[t]) * self.energyContentH2_LHV / self.effElec / self.world.dt + (self.maxSOC - self.dictSOC[t]) * self.comprCons), 0),  \
                                                (self.installedCapacity + self.installedCapacity * self.effElec/self.energyContentH2_LHV * self.world.dt * self.comprCons))
            # print(availablePower_H2_storage, self.name, 'availablePower_H2_storage')
            if availablePower_H2_storage >= self.world.minBidCRM:
                ...
                bidsCRM.append(Bid(issuer = self,
                                ...
                                status = "Sent",
                                bidType = "Supply"))
        else:  #t < 1 first timestep skipped
            bidsCRM.append(Bid(issuer = self,
                            ID = "{}_CRMNegDem".format(self.name),
                            price = 0,
                            amount = 0,
